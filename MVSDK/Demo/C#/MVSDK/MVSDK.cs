...
namespace MVSDK
{
    ...
    static public class MvApi
    {
		...
		/// \brief 获取绘制矩形
		/// \param [in] ZoomTool 缩放工具
		/// \param [out] FrameX   帧内矩形X坐标
		/// \param [out] FrameY   帧内矩形Y坐标
		/// \param [out] FrameW   帧内矩形宽度
		/// \param [out] FrameH   帧内矩形高度
		/// \param [out] ViewX    视口内矩形X坐标
		/// \param [out] ViewY    视口内矩形Y坐标
		/// \param [out] ViewW    视口内矩形宽度
		/// \param [out] ViewH    视口内矩形高度
		/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
		/// \~english
		...
		/// \brief 坐标变换
		/// \param [in] ZoomTool 缩放工具
		/// \param [in] Type     变换类型   0: 视口转帧坐标     1：视口转世界坐标(帧坐标*缩放因子)		2: 帧坐标转视口
