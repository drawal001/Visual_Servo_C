...
Begin VB.Form Form1 
   ...
   Begin VB.CommandButton ButtonPause 
      ...
      Width           =   1215
   End
   Begin eVision.EBW8Image EBW8Image1 
      Height          =   7215
      Left            =   240
      TabIndex        =   4
      Top             =   840
...
End If
    iRet = InitCameraForEvision(CameraList(0))
...
Dim Pic As IPictureDisp
    ...
    If iRet = CAMERA_STATUS_SUCCESS Then
        ...
        If iRet = CAMERA_STATUS_SUCCESS Then '处理成功，则显示该图像，这里使用了SDK封装的接口进行图像显示，您也可以自己实现图像的显示，或者直接在这里调用保存图像的接口进行保存
           ...
           EBW8Image1.SetImagePointer m_pFrameBuffer
           EBW8Image1.SetSize CLng(sFrameInfo.iWidth), CLng(sFrameInfo.iHeight)
           EBW8Image1.ZoomX = CSng(EBW8Image1.Width / sFrameInfo.iWidth)
           EBW8Image1.ZoomY = CSng(EBW8Image1.Height / sFrameInfo.iHeight)
