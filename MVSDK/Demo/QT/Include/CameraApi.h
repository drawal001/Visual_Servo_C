...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 设置多重曝光的模拟增益值。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [in] iGain要设定的模拟增益值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
...
MVSDK_API CameraSdkStatus __stdcall CameraSetMultiExposureGain(
	CameraHandle    hCamera, 
	int				index,
	int             iGain
	);
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的模拟增益值。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [out] piGain 指针，返回的模拟增益值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
...
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
/// \see CameraSetMultiExposureGain
MVSDK_API CameraSdkStatus __stdcall CameraGetMultiExposureGain(
	CameraHandle    hCamera, 
	int				index,
	int*            piGain
	);
