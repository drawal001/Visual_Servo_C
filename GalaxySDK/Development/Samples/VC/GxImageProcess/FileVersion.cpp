...
CString CFileVersion::GetVer(CString strSubBlock)
{
	...
	// 获取文件版本信息大小
	dwInfoSize = GetFileVersionInfoSize(const_cast<char*>((LPCTSTR)m_strPath), &dwHandle);

	if(dwInfoSize != 0)
	{
		// 获取版本信息
		pData = new char[dwInfoSize];
		if( GetFileVersionInfo(const_cast<char*>((LPCTSTR)m_strPath), 0, dwInfoSize, pData))
		{
